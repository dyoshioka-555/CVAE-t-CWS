...
class attnVAE:
    def __init__(
        ...
        self.train_data, self.train_label, self.train_bow = train
        if not debug_nan and not debug:
            self.valid_data, self.valid_label, self.valid_bow = valid
            self.test_data, self.test_label, self.test_bow = test
    ...
    def train(self, epoch):
        ...
        for idx in np.random.permutation(range(self.nbatch)):
            ...
            if 0:
                if shift > 0:
                    source_words = [
                        " ".join([self.vocab.id2word(id) for id in ids])
                    ...
                    for s, b, t in zip(source_words, batch_bow, target_words):
    ...
    def evaluate(self, eval_data, eval_label, eval_bow):
        self.vae.eval()
        ...
        with torch.no_grad():
            ...
            for batch_data, batch_label, batch_bow in zip(
                ...
                vae_logits, vae_kl1_loss, _ = self.vae.loss(
                    batch_data, batch_label, batch_bow, shift
                )
                vae_logits = vae_logits.view(-1, vae_logits.size(2))
                vae_rec_loss = F.cross_entropy(
                    vae_logits, target.view(-1), reduction="none"
    ...
    def fit(self):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            val_loss = self.evaluate(self.valid_data, self.valid_label, self.valid_bow)
