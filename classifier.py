...
class CNNClassifier_from_enc(nn.Module):
    def __init__(self, vocab_size, embed_dim, filter_sizes, n_filters, dropout):
        super(CNNClassifier, self).__init__()
        self.n_filters = n_filters
...
def main(args):
    ...
    ft_embed = np.zeros((len(vocab), 300))
    with open("data/cc.ja.300.vec") as f:
        for line in f:
            word, vec = line.split(" ", 1)
            if word in vocab:
                wid = vocab[word]
                ft_embed[wid, :] = np.fromstring(vec, sep=" ", dtype=np.float32)

        _mu = ft_embed.mean()
        _std = ft_embed.std()
        ft_embed[:4, :] = np.random.randn(4, 300) * _std + _mu

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    train_batch, train_label = train_data.create_data_batch_label(
        64, device, batch_first=True
    )
    dev_batch, dev_label = dev_data.create_data_batch_label(
        64, device, batch_first=True
    )
    test_batch, test_label = test_data.create_data_batch_label(
        64, device, batch_first=True
    )
    ...
    with torch.no_grad():
        model.embedding.weight.fill_(0.0)
        model.embedding.weight += torch.FloatTensor(ft_embed).to(device)
